...
@if (session()->has('error'))
    ...
    <form action="{{route('nedmin.Authenticate')}}" method="post">
      ...
      <div class="form-group has-feedback">
        <input type="email" class="form-control" required name="email" value="{{old('email')}}" placeholder="Kullanıcı Adı veya E-mail">
      ...
      <div class="row">
        <div class="col-xs-8">
          <div class="checkbox icheck">
            <label>
              <input type="checkbox" {{old('remember_me')=='on' ? 'checked' : ''}} name="remember_me"> Beni Hatırla
            </label>
          </div>
        </div>
        <!-- /.col -->
